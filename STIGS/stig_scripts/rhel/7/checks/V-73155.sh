...
# THIS CHECKS TO SEE IF THE SCREENSAVER LOCK DELAY IS SET

DIR_LIST=$1
                         
for CHK_DIR in $DIR_LIST 
  do 
    if [[ ! -z `echo -e $(grep -i lock-delay $CHK_DIR)` && ! -d $CHK_DIR ]];  then 
        FILE=$CHK_DIR
        NOTES="THIS REQUIREMENT IS NOT APPLICABLE IF SYSTEM DOES NOT HAVE GNOME INSTALLED"
        FOUND_ITEM=`echo -e $(grep -i lock-delay $CHK_DIR)`
